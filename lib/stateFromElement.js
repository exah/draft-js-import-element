...
var _cssToObject = require('./lib/cssToObject');

var _cssToObject2 = _interopRequireDefault(_cssToObject);
...
var _styleToCssString = require('./styleToCssString');
    ...
    for (var i = 0; i < element.attributes.length; i++) {
      ...
      if (value != null) {
        if (attrMap.hasOwnProperty(name)) {
          ...
          var newValue = name === 'style' ? (0, _cssToObject2.default)(value) : value;
...
var ELEM_TO_ENTITY = {
  ...
  span: function span(tagName, element) {
    ...
    if (data.style != null) {
      return _draftJs.Entity.create('INLINE_STYLES', 'MUTABLE', data);
    }
  },
      ...
      if (styleAttribute) {
        (function () {
          ...
          Object.keys(_this.customStyleMap).forEach(function (key) {
            customCssMapToStyle[(0, _styleToCssString2.default)(_this.customStyleMap[key])] = key;
          });
...
function addStyleFromStyleAttribute(styleSet, styleAttributeValue, customCssMapToStyle) {
  ...
  var stylesMapKeys = Object.keys(customCssMapToStyle);

  styleSet = stylesMapKeys.reduce(function (prevStyleSet, cssRule) {
